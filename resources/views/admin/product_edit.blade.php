...
<div class="card">
    ...
    <div class="card-body">
        <form class="row" action="{{ route("admin.product.update") }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method("post")
            <input type="hidden" name="product_id" value="{{ $product->id }}">
            <div class="form-group col-md-4">
                <img class="w-100" src="{{ asset("uploads/product") }}/{{ $product->image }}" alt="">
            </div>
            ...
            <div class="form-group col-md-6">
                ...
                <input type="text" class="form-control" id="slug" placeholder="Slug" name="slug" value="{{ $product->slug }}" readonly>
            </div>
            <div class="form-group col-md-6">
                <label for="price">Price*</label>
                <input name="price" type="text" class="form-control" id="price" placeholder="Price" value="{{ $product->price }}">
            ...
            <div class="form-group col-6">
                ...
                <select id="options" name="options[]" class="form-control select2" multiple>
                    @foreach($options as $option)
                        <option @if(in_array($option->id,explode(",",$product->options))) selected @endif value="{{ $option->id }}">{{ $option->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                ...
                <input name="max_quantity" type="text" class="form-control" id="max_quantity" placeholder="Quantity" value="{{ $product->max_quantity }}">
                @error("max_quantity") <small class="form-text text-danger">{{ $message }}</small> @enderror
            </div>
            <div class="form-group col-md-6">
                <label for="offer_price">Offer Price</label>
                <input name="offer_price" type="text" class="form-control" id="offer_price" placeholder="Offer Price" value="{{ $product->offer_price }}">
                @error("offer_price") <small class="form-text text-danger">{{ $message }}</small> @enderror
            </div>
            <div class="form-group col-md-6">
                <label for="seo_title">Seo Title</label>
                <input name="seo_title" type="text" class="form-control" id="seo_title" placeholder="Seo Title" value="{{ $product->seo_title }}">
                @error("seo_title") <small class="form-text text-danger">{{ $message }}</small> @enderror
            </div>
            <div class="form-group col-md-6">
                <label for="seo_description">Seo Description</label>
                <input name="seo_description" type="text" class="form-control" id="seo_description" placeholder="Seo Description" value="{{ $product->seo_description }}">
                @error("seo_description") <small class="form-text text-danger">{{ $message }}</small> @enderror
            </div>
            <div class="form-group col-md-6">
                <label for="short_description">Short Description</label>
                <input name="short_description" type="text" class="form-control" id="short_description" placeholder="Short Description" value="{{ $product->short_description }}">
                @error("short_description") <small class="form-text text-danger">{{ $message }}</small> @enderror
            </div>
            <div class="form-group col-12">
                <label for="description">Description*</label>
                <textarea name="description" type="text" class="form-control summernote" id="description" placeholder="Description">{{ $product->description }}</textarea>
