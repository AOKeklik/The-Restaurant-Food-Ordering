<div class="fp__menu_cart_header">
    <h5>total item (<span class="cart_item_count_get">{{ cartItemCount() }}</span>)</h5>
...
@if(Session::has("cart"))
    ...
    @foreach(Session::get("cart") as $key=>$val)
        <li>
            ...
            <div class="menu_cart_text">
                ...
                @endif
                <p class="price">
                    {{ currency(cartItemTotal($val["product_info"]["id"])) }}
                </p>
            </div>
    ...
    <p class="subtotal">sub total <span>{{ currency(cartTotal()) }}</span></p>
